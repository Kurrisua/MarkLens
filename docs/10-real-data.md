...
为了让真实数据源具备足够的检索规模，项目还支持连续下载最近 45 天内实际发布的 IPO CZ 日增量包：

```bash
make import-real-batch
```

该命令只访问 IPO CZ 固定的官方开放数据域名。下载器会跳过官方未发布的日期，并在落盘前校验 ZIP 中存在 ST.96 XML；批次导入器会保留每条记录对应的官方发布日期、元数据 URL 与原始 ZIP 文件名。相同申请号在不同日包中出现时，以同一真实数据源内的幂等更新方式保留较新的批次内容，而不是重复创建。

截至本次数据扩容，已取得 2026-07-05 至 2026-07-21 的 17 个真实官方日增量包：解析到 3,327 条记录、按申请号去重为 3,068 条，覆盖 Nice 1–45 类。原始 ZIP 保存在 `data/raw/ipo-cz-daily/` 且已被 Git 忽略；它们是官方数据缓存，不会被提交进代码仓库。
